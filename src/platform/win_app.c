...
static CI_BOOL IsCrossStation = CI_FALSE;
/*
功能描述    : 获取联锁周期数
返回值      : 
参数        : 
...
{
    int32_t ret = 0;
    char * is_cross_station = NULL;
    static CI_BOOL b_initialized = CI_FALSE;

    static char *softversion = NULL;
    ...
    if (CICentralized_IsDo(&IsCrossStation)<0)
    {
        Sleep(3 * 1000);
        return -1;
    }
    if (CIVersion_Get(&softversion) <0)
    {
        Sleep(3 * 1000);
        return -1;
    }
    CILog_Msg("Software Version：%s" , softversion);
    /*
    ...
    if (IsCrossStation == CI_TRUE)
    {
        ret = CICentralized_Init();
        assert(-1 != ret);
    }
