...
static int32_t broadcast = 1;

static int32_t hmi_sockto = 0;            /*向控显机通过PC104网口发送数据的文件句柄*/
static int32_t hmi_sockfrom = 0;            /*从控显机通过PC104网口接收数据的文件句柄*/

static struct sockaddr_in hmi_socket_addrto;            /*向控显机通过PC104网口发送数据的ip*/
static struct sockaddr_in hmi_socket_addrfrom;            /*从控显机通过PC104网口接收数据的ip*/
...
static uint16_t hmi_frame_sn = 0xffff;
static uint16_t hmi_nodedata_fsn = 0xffff;
...
static int32_t hmi_direct_write(const void* buf,int32_t nbytes)
{
    ...
    {
        //PC104网口
        sendto(hmi_sockto,buf,nbytes,0,(struct sockaddr *)&hmi_socket_addrto,sizeof(hmi_socket_addrto));
        write_num = write(hmi_fd_a,buf,nbytes);     //网络板
        CIMonitor_Write(buf,nbytes);    //性能检测
    }
    else
    {
        return 0;
...
#define HMI_SEND_MEMCPY(data)                       \
    ...
    eeu_channel_status_a = CIEeu_IsChannelAforHmi();
    eeu_channel_status_b = CIEeu_IsChannelBforHmi();
#endif /* WIN32*/
...
static int32_t hmi_read(int32_t fd, HmiRecieveFrame* frame)
{
    ...
    {
        read_num = recvfrom(hmi_sockfrom, frame, sizeof(*frame), 0, NULL, NULL);
    }
#endif /*WIN32*/
    if (0 >= read_num)
    {
    ...
    {
        /*只有主系主CPU检查帧序号,其它的一律不检查,输入同步时会保证一致*/
        if (abs(frame->head.frame_sn - hmi_nodedata_fsn) > 3 
            && CPU_STATE_MASTER == CICpu_GetLocalState()
            && SERIES_MASTER == CISeries_GetLocalState())
        {
            CILog_Msg("hmi frame fsn:%#x,%#x",frame->head.frame_sn,hmi_nodedata_fsn);
...
int32_t CIHmi_RecvData(void)
{
	...
	/*清除udp接受缓存*/
	while(0 < recvfrom(hmi_sockfrom, dummy_buf, 65535, 0, NULL, NULL));
    ...
    case DT_HMI_SWITCH_COMMAND:
        ...
        if (SERIES_MASTER == CISeries_GetLocalState())
        {
            ...
            {
                CIHmi_SendNormalTips("联锁机收到控显机切换命令，正在切换");
                /*强制进行切换*/
...
/*PC104与控显机socket通信初始化*/
static int32_t hmi_socket_int(void)
{
    int flags;

    hmi_sockto = socket(AF_INET, SOCK_DGRAM, 0);
    if (-1 == hmi_sockto)
    {
        CILog_Errno("socket hmi_sockto failed");

        return -1;
    }

    /*广播*/
    if (-1 == setsockopt(hmi_sockto, SOL_SOCKET, SO_BROADCAST, &broadcast, sizeof(broadcast)))
    {
        CILog_Errno("设置hmi_sockto为广播失败");
        return -1;
    }

    bzero(&hmi_socket_addrto, sizeof(struct sockaddr_in));
    hmi_socket_addrto.sin_family = AF_INET;
    hmi_socket_addrto.sin_addr.s_addr = inet_addr("192.168.1.255");;
    hmi_socket_addrto.sin_port = htons(hmi_port_a);

    hmi_sockfrom = socket(AF_INET, SOCK_DGRAM, 0);
    if (-1 == hmi_sockfrom)
    {
        CILog_Errno("socket hmi_sockfrom failed");
        return -1;
    }
    bzero(&hmi_socket_addrfrom, sizeof(struct sockaddr_in));
    hmi_socket_addrfrom.sin_family = AF_INET;
    hmi_socket_addrfrom.sin_addr.s_addr = htonl(INADDR_ANY);
    hmi_socket_addrfrom.sin_port = htons(3001);

    /*绑定*/
    if(bind(hmi_sockfrom,(struct sockaddr *)&(hmi_socket_addrfrom), sizeof(struct sockaddr_in)) == -1)   
    {     
        CILog_Errno("socket bind failed");
        return -1;
    }
    flags = fcntl(hmi_sockfrom,F_GETFL,0);

    fcntl(hmi_sockfrom,F_SETFL,flags | O_NONBLOCK);
