...
static int32_t eeuA_lost_count = 0 ; /*电子单元A口失去连接次数*/
static int32_t eeuB_lost_count = 0 ; /*电子单元B口失去连接次数*/
...
static int32_t eeu_recv_data_ext(int32_t fd,EeuFrame* frame,const char* UNUSED(fd_name))
{
    ...
    else
    {
        ...
        {
            if(frame->id.tag.fsn != this_request_fsn)
            {
                if (CI_TRUE == b_print_eeu)
                {
                    eeu_recv_log_buf_len += snprintf(eeu_recv_log_buf + eeu_recv_log_buf_len,
...
{
    static int sja_err_count_a = 0;
    static int sja_err_count_b = 0;
        ...
        if (CI_FALSE == eeu_check_sja_status())
        {
            if (40 < CITimer_GetSystemStartCounter())
            {
                if(CPU_STATE_SLAVE == CICpu_GetLocalState()
                   ...
                   && sja_err_count_a % 100 == 2)
                {
                    CILog_Msg("主系从CPU正在复位sja1000 A");
                    ioctl(eeu_fd_a, CAN_IOCRESET);
                }
                /*让主系与备系的复位时间错开,因为当电子单元通信断开时需要由备系判断是否切换*/
                if(CPU_STATE_SLAVE == CICpu_GetLocalState()
                   && SERIES_STANDBY == CISeries_GetLocalState()
                   && sja_err_count_a % 100 == 20)
                {
                    CILog_Msg("备系从CPU正在复位sja1000 A");
                    ioctl(eeu_fd_a, CAN_IOCRESET);
                }
            }
    ...
    if (CI_FALSE == is_channel_b_ok)
    {
        ...
        if (CI_FALSE == eeu_check_sja_status() && CPU_STATE_MASTER == CICpu_GetLocalState())
        {
            if (40 < CITimer_GetSystemStartCounter())
            {
                if(CPU_STATE_MASTER == CICpu_GetLocalState()
                   && SERIES_MASTER == CISeries_GetLocalState()
                   && sja_err_count_b % 100 == 2)
                {
                    CILog_Msg("主系主CPU正在复位sja1000 B");
                    ioctl(eeu_fd_b, CAN_IOCRESET);
                }
                /*让主系与备系的复位时间错开,因为当电子单元通信断开时需要由备系判断是否切换*/
                if(CPU_STATE_MASTER == CICpu_GetLocalState()
                   && SERIES_STANDBY == CISeries_GetLocalState()
                   && sja_err_count_b % 100 == 20)
                {
                    CILog_Msg("备系主CPU正在复位sja1000 B");
                    ioctl(eeu_fd_b, CAN_IOCRESET);
                }
            }
        }
...
/*
功能描述    : 得到A通信道的状态,用于控显机状态显示
返回值      : 通信道正常则返回CI_TRUE,否则返回CI_FALSE
参数        : 无
日期        : 2015年5月29日 18:30:00
*/
CI_BOOL CIEeu_IsChannelAforHmi(void)
{
    CI_BOOL ret = CI_FALSE ;
    if(eeuA_lost_count <3)
        ret = CI_TRUE ;
    return ret;
}
/*
功能描述    : 得到B通信道的状态，用于控显机状态显示
返回值      : 通信道正常则返回CI_TRUE,否则返回CI_FALSE
参数        : 无
日期        : 2015年5月29日 18:30:00
*/
CI_BOOL CIEeu_IsChannelBforHmi(void)
{
    CI_BOOL ret = CI_FALSE ;
    if(eeuB_lost_count <3)
        ret = CI_TRUE ;
    return ret;
}
/*
