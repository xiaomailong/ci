...
typedef uint16_t InputUint16;
typedef int16_t InputInt16;
...
/*普通道口同步数据*/
typedef struct _InputHignCross
{
	CI_BOOL AlarmState;
}InputHignCross;
...
/*跨场作业相关数据*/
typedef struct _InputCrossStationStartEnd
{
	char_t LinkSignal[TEST_NAME_LENGTH];
	node_t StartSignal;
	CI_BOOL IsRetun;
	CI_BOOL LockSection;
}InputCrossStationStartEnd;
typedef struct _InputCrossStationEndStart
{
	char_t LinkSignal[TEST_NAME_LENGTH];
	node_t StartSignal;
	EN_route_state RouteState;
}InputCrossStationEndStart;
typedef struct _InputCrossStationRoute
{
	CI_BOOL IsExist;	/*是否存在虚拟进路*/
}InputCrossStationRoute;
...
typedef struct _CIInputSynFrame
{
	...
	InputUint16 button_locked[MAX_BUTTON_LOCKED];  /*按钮封锁*/
	InputInt16 train_loss_in_section[MAX_TRAIN_LOSS];  /*区段分路不良*/
	InputHignCross_bgfylz65 dk_bgfylz65_config;			 /*宝钢付原料站65#道口*/
	InputHignCross dk_config[MAX_HIGH_CROSS];			 /*普通道口*/

	/*站场1*/
	InputCrossStationEndStart CrossStation1SendToStart[MAX_CROSS_STATION];/*跨场作业终端-->始端发送数据*/
	InputCrossStationEndStart CrossStation1RecviceFromEnd[MAX_CROSS_STATION];/*跨场作业接收终端数据*/
	InputCrossStationStartEnd CrossStation1SendToEnd[MAX_CROSS_STATION];/*跨场作业始端-->终端发送数据*/
	InputCrossStationStartEnd CrossStation1RecviceFromStart[MAX_CROSS_STATION];/*跨场作业接收始端数据*/
	/*站场2*/
	InputCrossStationEndStart CrossStation2SendToStart[MAX_CROSS_STATION];/*跨场作业终端-->始端发送数据*/
	InputCrossStationEndStart CrossStation2RecviceFromEnd[MAX_CROSS_STATION];/*跨场作业接收终端数据*/
	InputCrossStationStartEnd CrossStation2SendToEnd[MAX_CROSS_STATION];/*跨场作业始端-->终端发送数据*/
	InputCrossStationStartEnd CrossStation2RecviceFromStart[MAX_CROSS_STATION];/*跨场作业接收始端数据*/
	/*虚拟进路*/
	InputCrossStationRoute CrossStationRoute[MAX_CROSS_STATION];/*跨场作业虚拟进路数据*/
