...
int32_t CIInputSyn_Assemble(CIInputSynFrame* p_frame)
{
    ...
    */
	...
	/*装配区段分路不良数据*/
	for (i = 0; i < MAX_TRAIN_LOSS; i++)
	{
		p_frame->train_loss_in_section[i] = train_loss_in_section[i];
	}
	...
	{
		p_frame->dk_config[i].AlarmState = dk_config[i].AlarmState;
	}

	/*装配跨场作业数据*/
	for (i = 0; i < MAX_CROSS_STATION; i++)
	{
		p_frame->CrossStation1SendToStart[i].RouteState = CrossStation1SendToStart[i].RouteState;
		for (j = 0; j < TEST_NAME_LENGTH; j ++)
		{
			p_frame->CrossStation1RecviceFromEnd[i].LinkSignal[j] = CrossStation1RecviceFromEnd[i].LinkSignal[j];
		} 
		p_frame->CrossStation1RecviceFromEnd[i].StartSignal = CrossStation1RecviceFromEnd[i].StartSignal;
		p_frame->CrossStation1RecviceFromEnd[i].RouteState = CrossStation1RecviceFromEnd[i].RouteState;		
		p_frame->CrossStation1SendToEnd[i].IsRetun = CrossStation1SendToEnd[i].IsRetun;
		for (j = 0; j < TEST_NAME_LENGTH; j ++)
		{
			p_frame->CrossStation1RecviceFromStart[i].LinkSignal[j] = CrossStation1RecviceFromStart[i].LinkSignal[j];
		} 
		p_frame->CrossStation1RecviceFromStart[i].StartSignal = CrossStation1RecviceFromStart[i].StartSignal;
		p_frame->CrossStation1RecviceFromStart[i].IsRetun = CrossStation1RecviceFromStart[i].IsRetun;
		p_frame->CrossStation1RecviceFromStart[i].LockSection = CrossStation1RecviceFromStart[i].LockSection;

		p_frame->CrossStation2SendToStart[i].RouteState = CrossStation2SendToStart[i].RouteState;
		for (j = 0; j < TEST_NAME_LENGTH; j ++)
		{
			p_frame->CrossStation2RecviceFromEnd[i].LinkSignal[j] = CrossStation2RecviceFromEnd[i].LinkSignal[j];
		} 
		p_frame->CrossStation2RecviceFromEnd[i].StartSignal = CrossStation2RecviceFromEnd[i].StartSignal;
		p_frame->CrossStation2RecviceFromEnd[i].RouteState = CrossStation2RecviceFromEnd[i].RouteState;
		p_frame->CrossStation2SendToEnd[i].IsRetun = CrossStation2SendToEnd[i].IsRetun;
		for (j = 0; j < TEST_NAME_LENGTH; j ++)
		{
			p_frame->CrossStation2RecviceFromStart[i].LinkSignal[j] = CrossStation2RecviceFromStart[i].LinkSignal[j];
		} 
		p_frame->CrossStation2RecviceFromStart[i].StartSignal = CrossStation2RecviceFromStart[i].StartSignal;
		p_frame->CrossStation2RecviceFromStart[i].IsRetun = CrossStation2RecviceFromStart[i].IsRetun;
		p_frame->CrossStation2RecviceFromStart[i].LockSection = CrossStation2RecviceFromStart[i].LockSection;

		p_frame->CrossStationRoute[i].IsExist = CrossStationRoute[i].IsExist;
	}
...
#define SERIES_INPUT_ARRAY_CMP(arr,index,field)                                                     \
	...
	for (i = 0; i < MAX_TRAIN_LOSS; i++)
	{
		if (self_input_frame.train_loss_in_section[i] != peer_input_frame.train_loss_in_section[i])
		{
			CIRemoteLog_Write("input_cmp_train_loss_in_section_fail:%d_%d:%#x!=%#x",
				i, j, self_input_frame.train_loss_in_section[i], peer_input_frame.train_loss_in_section[i]);
			return -1;
		}
	}
	...
	for (i = 0; i < MAX_HIGH_CROSS; i++)
	{
		if (self_input_frame.dk_config[i].AlarmState != peer_input_frame.dk_config[i].AlarmState)
		{
			CIRemoteLog_Write("input_cmp_dk_config_fail:%d_%d:%#x!=%#x",
				i, j, self_input_frame.dk_config[i].AlarmState, peer_input_frame.dk_config[i].AlarmState);
			return -1;
		}	
	}
	...
	for (i = 0; i < MAX_CROSS_STATION; i++)
	{
		SERIES_INPUT_ARRAY_CMP(CrossStation1SendToStart, i, RouteState);
		for (j = 0; j < TEST_NAME_LENGTH; j ++)
        {
			if(self_input_frame.CrossStation1RecviceFromEnd[i].LinkSignal[j] != peer_input_frame.CrossStation1RecviceFromEnd[i].LinkSignal[j])
			{
				CIRemoteLog_Write("input_cmp_CrossStation1RecviceFromEnd_LinkSignal_fail:%d_%d:%#x!=%#x",
				i, j, self_input_frame.CrossStation1RecviceFromEnd[i].LinkSignal[j], peer_input_frame.CrossStation1RecviceFromEnd[i].LinkSignal[j]);
				return -1;
			}
        } 
		SERIES_INPUT_ARRAY_CMP(CrossStation1RecviceFromEnd, i, StartSignal);
		SERIES_INPUT_ARRAY_CMP(CrossStation1RecviceFromEnd, i, RouteState);
		SERIES_INPUT_ARRAY_CMP(CrossStation1SendToEnd, i, IsRetun);
		for (j = 0; j < TEST_NAME_LENGTH; j ++)
		{
			if(self_input_frame.CrossStation1RecviceFromStart[i].LinkSignal[j] != peer_input_frame.CrossStation1RecviceFromStart[i].LinkSignal[j])
			{
				CIRemoteLog_Write("input_cmp_CrossStation1RecviceFromStart_LinkSignal_fail:%d_%d:%#x!=%#x",
					i, j, self_input_frame.CrossStation1RecviceFromStart[i].LinkSignal[j], peer_input_frame.CrossStation1RecviceFromStart[i].LinkSignal[j]);
				return -1;
			}
		} 
		SERIES_INPUT_ARRAY_CMP(CrossStation1RecviceFromStart, i, StartSignal);
		SERIES_INPUT_ARRAY_CMP(CrossStation1RecviceFromStart, i, IsRetun);
		SERIES_INPUT_ARRAY_CMP(CrossStation1RecviceFromStart, i, LockSection);

		SERIES_INPUT_ARRAY_CMP(CrossStation2SendToStart, i, RouteState);
		for (j = 0; j < TEST_NAME_LENGTH; j ++)
		{
			if(self_input_frame.CrossStation2RecviceFromEnd[i].LinkSignal[j] != peer_input_frame.CrossStation2RecviceFromEnd[i].LinkSignal[j])
			{
				CIRemoteLog_Write("input_cmp_CrossStation2RecviceFromEnd_LinkSignal_fail:%d_%d:%#x!=%#x",
					i, j, self_input_frame.CrossStation2RecviceFromEnd[i].LinkSignal[j], peer_input_frame.CrossStation2RecviceFromEnd[i].LinkSignal[j]);
				return -1;
			}
		} 
		SERIES_INPUT_ARRAY_CMP(CrossStation2RecviceFromEnd, i, StartSignal);
		SERIES_INPUT_ARRAY_CMP(CrossStation2RecviceFromEnd, i, RouteState);
		SERIES_INPUT_ARRAY_CMP(CrossStation2SendToEnd, i, IsRetun);
		for (j = 0; j < TEST_NAME_LENGTH; j ++)
		{
			if(self_input_frame.CrossStation2RecviceFromStart[i].LinkSignal[j] != peer_input_frame.CrossStation2RecviceFromStart[i].LinkSignal[j])
			{
				CIRemoteLog_Write("input_cmp_CrossStation2RecviceFromStart_LinkSignal_fail:%d_%d:%#x!=%#x",
					i, j, self_input_frame.CrossStation2RecviceFromStart[i].LinkSignal[j], peer_input_frame.CrossStation2RecviceFromStart[i].LinkSignal[j]);
				return -1;
			}
		} 
		SERIES_INPUT_ARRAY_CMP(CrossStation2RecviceFromStart, i, StartSignal);
		SERIES_INPUT_ARRAY_CMP(CrossStation2RecviceFromStart, i, IsRetun);
		SERIES_INPUT_ARRAY_CMP(CrossStation2RecviceFromStart, i, LockSection);

		SERIES_INPUT_ARRAY_CMP(CrossStationRoute, i, IsExist);
	}
...
int32_t CIinputSyn_Replace(const CIInputSynFrame* p_frame)
{
	...
	for (i = 0; i < MAX_TRAIN_LOSS; i++)
	{
		train_loss_in_section[i] = p_frame->train_loss_in_section[i];
	}
	...
	for (i = 0; i < MAX_HIGH_CROSS; i++)
	{
		dk_config[i].AlarmState = p_frame->dk_config[i].AlarmState;
	}

	/*替换跨场作业数据*/
	for (i = 0; i < MAX_CROSS_STATION; i++)
	{
		CrossStation1SendToStart[i].RouteState = p_frame->CrossStation1SendToStart[i].RouteState;
		for (j = 0; j < TEST_NAME_LENGTH; j ++)
		{
			CrossStation1RecviceFromEnd[i].LinkSignal[j] = p_frame->CrossStation1RecviceFromEnd[i].LinkSignal[j];
		} 
		CrossStation1RecviceFromEnd[i].StartSignal = p_frame->CrossStation1RecviceFromEnd[i].StartSignal;
		CrossStation1RecviceFromEnd[i].RouteState = p_frame->CrossStation1RecviceFromEnd[i].RouteState;		
		CrossStation1SendToEnd[i].IsRetun = p_frame->CrossStation1SendToEnd[i].IsRetun;
		for (j = 0; j < TEST_NAME_LENGTH; j ++)
		{
			CrossStation1RecviceFromStart[i].LinkSignal[j] = p_frame->CrossStation1RecviceFromStart[i].LinkSignal[j];
		} 
		CrossStation1RecviceFromStart[i].StartSignal = p_frame->CrossStation1RecviceFromStart[i].StartSignal;
		CrossStation1RecviceFromStart[i].IsRetun = p_frame->CrossStation1RecviceFromStart[i].IsRetun;
		CrossStation1RecviceFromStart[i].LockSection = p_frame->CrossStation1RecviceFromStart[i].LockSection;

		CrossStation2SendToStart[i].RouteState = p_frame->CrossStation2SendToStart[i].RouteState;
		for (j = 0; j < TEST_NAME_LENGTH; j ++)
		{
			CrossStation2RecviceFromEnd[i].LinkSignal[j] = p_frame->CrossStation2RecviceFromEnd[i].LinkSignal[j];
		} 
		CrossStation2RecviceFromEnd[i].StartSignal = p_frame->CrossStation2RecviceFromEnd[i].StartSignal;
		CrossStation2RecviceFromEnd[i].RouteState = p_frame->CrossStation2RecviceFromEnd[i].RouteState;
		CrossStation2SendToEnd[i].IsRetun = p_frame->CrossStation2SendToEnd[i].IsRetun;
		for (j = 0; j < TEST_NAME_LENGTH; j ++)
		{
			CrossStation2RecviceFromStart[i].LinkSignal[j] = p_frame->CrossStation2RecviceFromStart[i].LinkSignal[j];
		} 
		CrossStation2RecviceFromStart[i].StartSignal = p_frame->CrossStation2RecviceFromStart[i].StartSignal;
		CrossStation2RecviceFromStart[i].IsRetun = p_frame->CrossStation2RecviceFromStart[i].IsRetun;
		CrossStation2RecviceFromStart[i].LockSection = p_frame->CrossStation2RecviceFromStart[i].LockSection;
		
		CrossStationRoute[i].IsExist = p_frame->CrossStationRoute[i].IsExist;
	}
