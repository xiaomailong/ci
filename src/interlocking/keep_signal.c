		...
		else
		{
			...
			if (gr_error_count(route_index) > MAX_ERROR_PER_COMMAND)
			{
				...
				else
				{
					...
					sr_clear_error_count(route_index);
					process_warning(ERR_COMMAND_EXECUTE,gr_name(route_index));
					CIHmi_SendNormalTips("信号转换失败：%s",gn_name(gr_start_signal(route_index)));
				}	
			}				
		}
		/*设置信号机的历史状态*/
...
EN_signal_state change_signal_color(route_t route_index)
{
	...
	if (IsTRUE(is_route_exist(route_index)))
	{
		...
		{
			if ((signal_show_change_config[i].StartSignal == gr_start_signal(route_index))
...
EN_signal_state out_signal_color_definition( route_t route_index )
{
	...
	if (IsTRUE(is_route_exist(route_index)))
	{
			...
			|| (gn_type(end_signal) == NT_ROUTE_SIGNAL))
		{
			...
			{								
				/*对方站同意接车或出站信号允许开放*/
				if (SAB_FIRST_SECTION_CLEAR2 == gsab_state(temp) || SAB_OUT_SIGNAL_OPENED  == gsab_state(temp))
				{
					result = SGS_L;
				}
				else
	...
	if (IsTRUE(is_route_exist(route_index)))
	{
		...
		for (i = 0; i < count; i++)
		{
			...
			if (i == (count - 1) 
				...
				&& ((gn_type(gr_start_signal(route_index)) == NT_ROUTE_SIGNAL) || (gn_type(gr_start_signal(route_index)) == NT_JUXTAPOSE_SHUNGTING_SIGNAL)
				|| (gn_type(gr_start_signal(route_index)) == NT_DIFF_SHUNTING_SIGNAL) || (gn_type(gr_start_signal(route_index)) == NT_STUB_END_SHUNTING_SIGNAL))
...
CI_BOOL is_relation_condition_ok(route_t route_index)
{
	...
	if (IsTRUE(is_route_exist(route_index)))
	{
		...
		else if ((temp = gs_special(route_index,SPT_SEMI_AUTO_BLOCK)) != NO_INDEX)
		{
			...
			if ((SAB_ERROR <= gsab_state(temp)) && (SAB_START_RECIEVED_AGREEMENT >= gsab_state(temp)))
			{
				CIHmi_SendNormalTips("未办理半自动");
			}
			if ((SAB_DEPARTED <= gsab_state(temp)) && (SAB_DEPA_START_RECIEVED_CANCEL >= gsab_state(temp)))
			{
				CIHmi_SendNormalTips("区间有车");
			}
		}
		/*hjh 2013-8-20 场间联系*/
