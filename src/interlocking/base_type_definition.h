...
typedef enum semi_auto_block_state /*64D半自动闭塞状态*/
{
	...
	SAB_START_RECIEVED_AUTO_REPLY = 0xAB04,/*开始接收自动回执信息*/	
	SAB_RECIEVED_AUTO_REPLY = 0xAB05,  /*收到（—）/对方站已收到请求信息*/
	SAB_FIRST_SECTION_CLEAR1 = 0xAB06,/*进站信号机内方第一区段空闲1*/
	SAB_START_RECIEVED_AGREEMENT = 0xAB07,/*开始接收同意接车信息*/
	SAB_RECIEVED_AGREEMENT = 0xAB08,   /*收到（+）/已收到对方站同意接车信息*/
	SAB_FIRST_SECTION_CLEAR2 = 0xAB09,/*进站信号机内方第一区段空闲2*/
	SAB_OUT_SIGNAL_OPENED = 0xAB0A,    /*出站信号机已开放/出站信号已开放*/	
	SAB_DEPARTED = 0xAB0B,             /*出站信号机正常自动关闭/本站已发车*/	
	SAB_SENT_DEPARTED = 0xAB0C,        /*已发送本站通知出发信息*/	
	SAB_DEPA_START_RECIEVED_CANCEL = 0xAB0D,      /*开始接收解除闭塞信息/已收到解除闭塞信息*/	
	SAB_DEPA_RECIEVED_CANCEL = 0xAB0E,      /*接收到解除闭塞信息/已收到解除闭塞信息*/	
	/********************************************************************/
	SAB_START_RECIEVED_REQUEST = 0xAB0F,     /*开始接收对方站请求闭塞*/
	SAB_RECIEVED_REQUEST = 0xAB10,     /*收到(+)/对方站请求闭塞*/
	SAB_AGREED_RECIEVE_TRAIN = 0xAB11, /*已发送自动回执*/
	SAB_RECV_START_RECIEVED_CANCEL = 0xAB12, /*开始接收复原信息*/
	SAB_RECV_RECIEVED_CANCEL = 0xAB1B, /*复原信息接收完毕*/
	SAB_SENDING_AGREEMENT = 0xAB13,    /*发送（+）/同意接车信息正在发送*/
	SAB_SENT_AGREEMENT = 0xAB14,       /*同意接车信息已发送*/
	SAB_START_RECIEVED_DEPARTED = 0xAB15,    /*收到（+）/已收到对方站通知出发信息*/
	SAB_RECIEVED_DEPARTED = 0xAB16,    /*收到（+）/已收到对方站通知出发信息*/
	SAB_TRAIN_APPROACH = 0xAB17,       /*列车到达接近区段/列车接近*/ /*进站信号机开放/接车信号已开放*/
	SAB_TRAIN_IN_ENTRY_SIGNAL = 0xAB18,       /*列车进入进站信号机内方/列车已到达*/
	SAB_TRAIN_ARRIVED_TRACK = 0xAB19,  /*接车进路已解锁/列车到达股道*/	
		
	SAB_RECOVER = 0xAB1A,              /*按压复原或事故按钮/办理事故复原*/   
	SAB_START_RECIEVED_POSITIVE_MESSAGE = 0xAB1C,/*半自动未启用时开始接收(+)信息*/
	SAB_START_RECIEVED_NEGATIVE_MESSAGE = 0xAB1D/*半自动未启用时开始接收(-)信息*/
...
{            
	...
	RS_SO_SIGNAL_DELAY_OPENING   /*信号开放模块延时开放信号*/
}EN_route_state;
...
{ 
	...
	FB_BUTTON_UNCLOSE = -19,		/*17.按钮解封 */
	FB_HOLD_ROUTE = -20,			/*18.非进路控制 */
	FB_HOLD_ROUTE_FAULT = -21,		/*19.非进路故障恢复 */
	FB_TRAIN_LOSS_IN_SECTION = -22,	/*20.区段分路不良 */
	FB_INIT_DATA1 = -100,			/*初始化系统数据按钮*/
}EN_function_button;
