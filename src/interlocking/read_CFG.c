...
void WriteToSpecialConfig(char_t * SpecialName,uint8_t TabCount,char_t * TempBuff,uint8_t CommaCount)
{
	...
	/*上电启动锁闭道岔区段*/
	else if (strcmp_no_case(SpecialName,"SDSBQD") == 0)
	{
		if (strcmp_no_case(TempBuff,"Yes") == 0)
		{
			FirstRunLockedSwitchSection = CI_TRUE;
		}
		else
		{
			FirstRunLockedSwitchSection = CI_FALSE;
		}
	}
	/*半自动闭塞*/
	...
	else if (strcmp_no_case(SpecialName,"TSSR") == 0)
	{
		...
		else if (TabCount == 4)
		{
			if (strcmp_no_case(TempBuff,"Yes") == 0)
			{
				special_input_config[total_special_input].OutputNodeExist = CI_TRUE;
			}
			else if (strcmp_no_case(TempBuff,"No") == 0)
			{
				special_input_config[total_special_input].OutputNodeExist = CI_FALSE;
			}
			else
			{
				CIHmi_SendDebugTips("Warning!特殊输入配置错误！");
	...
	/*跨场作业*/
	else if (strcmp_no_case(SpecialName,"KCZY") == 0)
	{
		/*注意：CrossStation1SendToStart是工厂站和付原料站跨场作业
				CrossStation2SendToStart是工厂站和成品库站跨场作业*/
		if (total_cross_station == 0)
		{
			if (TabCount == 0)
			{
				strcat_check(CrossStation1SendToStart[total_cross_station].LinkSignal,TempBuff,sizeof(TempBuff));
				strcat_check(CrossStation1SendToEnd[total_cross_station].LinkSignal,TempBuff,sizeof(TempBuff));
			}
			else if (TabCount == 1)
			{
			...
			{
				CIHmi_SendDebugTips("Warning!跨场作业配置文件错误！");
			}
		}
		else if (total_cross_station == 1)
		{
			if (TabCount == 0)
			{
				strcat_check(CrossStation2SendToStart[total_cross_station - 1].LinkSignal,TempBuff,sizeof(TempBuff));
				strcat_check(CrossStation2SendToEnd[total_cross_station - 1].LinkSignal,TempBuff,sizeof(TempBuff));
			}
			else if (TabCount == 1)
			{
