...
日期：     2013/04/19
****************************************************/
CI_BOOL check_juxtapose_unlock_condition(route_t route_index)
{
...
日期：     2013/04/19
****************************************************/
CI_BOOL check_part_unlock_condition(route_t route_index)
{
	CI_BOOL result = CI_TRUE;
	node_t return_signal = NO_INDEX;
	route_t return_route = NO_INDEX;
...
日期：     2013/04/19
****************************************************/
route_t check_all_unlock_condition(route_t route_index)
{
	CI_BOOL result = CI_TRUE;
	route_t lead_route = NO_INDEX,return_route = NO_INDEX,temp_route = NO_INDEX;
	int16_t node,i,j,k,return_signal = NO_INDEX;
		...
		|| (RS_SK_N_SIGNAL_CLOSED != gr_state(lead_route)))
	{
		...
		/*hjh 2015-10-08 寻找跨场作业进路*/		
        if ((lead_route != NO_INDEX) && (RS_SK_N_SIGNAL_CLOSED == gr_state(lead_route)))
        {
            temp_route = lead_route;
        }
        else
        {
            lead_route = route_index;
            temp_route = route_index;
        }
		for (i = 0; i < MAX_ROUTE; i++)
		{
			if (gr_backward(temp_route) == NO_INDEX)
			{
				for (j = 0; j < MAX_CROSS_STATION; j++)
				{						
					if ((strcmp_no_case(CrossStation1RecviceFromStart[j].LinkSignal,gn_name(gr_start_signal(temp_route))) == 0)
						&& (IsTRUE(CrossStation1RecviceFromStart[j].IsRetun))
						&& (SCS_CLEARED == gn_section_state(gr_approach(lead_route))))
					{
						result = CI_TRUE;
						/*车列退出进路*/
						for (k = gr_nodes_count(lead_route) - 1; k >= 0; k--)
						{
							node = gr_node(lead_route,k);
							if (IsTRUE(is_section(node)) 
					...
					if ((strcmp_no_case(CrossStation2RecviceFromStart[j].LinkSignal,gn_name(gr_start_signal(temp_route))) == 0)
						&& (IsTRUE(CrossStation2RecviceFromStart[j].IsRetun))
						&& (SCS_CLEARED == gn_section_state(gr_approach(lead_route))))
					{
						result = CI_TRUE;
						/*车列退出进路*/
			...
			{
				temp_route = gr_backward(temp_route);
			}
		}
	}
	else
			...
			switch (gn_state_machine(current_section))
			{
				...
				case SCSM_INIT:
					...
					if ((bw_section == NO_INDEX) && (SCS_CLEARED != gn_section_state(current_section))
						...
						&& (fw_section != gr_last_section(route_index)))
					{
						sn_state_machine(current_section,SCSM_FAULT);
					...
					if ((bw_section != NO_INDEX) && (SCS_CLEARED != gn_section_state(bw_section))
						&& (fw_section != NO_INDEX) && (SCS_CLEARED != gn_section_state(fw_section))
						&& (SCS_CLEARED != gn_section_state(current_section)))
					{
						sn_state_machine(current_section,SCSM_FAULT);
						CIHmi_SendNormalTips("区段故障：%s",gn_name(current_section));
						break;
				...
				case SCSM_BEHIND_OCCUPIED:
					...
					if ((fw_section != NO_INDEX) && (SCS_CLEARED != gn_section_state(fw_section))
						...
						&& (gn_type(fw_section) != NT_TRACK)
						&& (gn_type(fw_section) != NT_NON_SWITCH_SECTION)
						&& (gn_type(fw_section) != NT_STUB_END_SECTION)
						&& (gn_type(fw_section) != NT_LOCODEPOT_SECTION)
						&& (gn_type(fw_section) != NT_APP_DEP_SECTION))
					{
						sn_state_machine(current_section,SCSM_FAULT);
						CIHmi_SendNormalTips("区段故障：%s",gn_name(current_section));
						break;
					...
					if ((bw_section != NO_INDEX) && (SCS_CLEARED == gn_section_state(bw_section)) 
						&& (current_section != gr_first_section(route_index))
						&& (SCSM_SELF_RETURN != gn_state_machine(bw_section))
						&& (RS_A_SIGNAL_CLOSED != gr_state(route_index)))
					{
						/*sn_state_machine(current_section,SCSM_FAULT_BEHIND);
						CIHmi_SendNormalTips("区段故障：%s",gn_name(current_section));*/

						/*hjh 2015-10-09 宝钢工厂站D122-->SL处的调车折返特殊处理*/
						if (((gn_type(bw_section) == NT_TRACK)
							|| (gn_type(bw_section) == NT_NON_SWITCH_SECTION)
							|| (gn_type(bw_section) == NT_STUB_END_SECTION)
							|| (gn_type(bw_section) == NT_LOCODEPOT_SECTION)
							|| (gn_type(bw_section) == NT_APP_DEP_SECTION))
							&& (RT_SHUNTING_ROUTE == gr_type(route_index))
							&& (SCSM_INIT == gn_state_machine(bw_section))
							)
						{
	...
	{							
		...
		if (bw_route != NO_ROUTE)
		{
			...
			else
			{
				...
				{
					if ((strcmp_no_case(CrossStation1RecviceFromStart[i].LinkSignal,gn_name(gr_start_signal(bw_route))) == 0))
					{
						if (IsTRUE(CrossStation1RecviceFromStart[i].IsRetun))
						{
							sn_state_machine(current_section,SCSM_SELF_RETURN);
						}
						else
					...
					if ((strcmp_no_case(CrossStation2RecviceFromStart[i].LinkSignal,gn_name(gr_start_signal(bw_route))) == 0))
					{
						if (IsTRUE(CrossStation2RecviceFromStart[i].IsRetun))
						{
							sn_state_machine(current_section,SCSM_SELF_RETURN);
						}
						else
		...
		for (i = 0; i < node_count; i++)
		{
			...
			if ( i > node_ordinal)
			{
				...
				if (gn_belong_route(current_node) == route_index)
				{
					...
					/*hjh 2015-9-10 针对特殊情况：尽头线前方还有无岔区段时也将其解锁*/
					if ((gn_type(current_node) == NT_STUB_END_SECTION ) || ((gn_type(current_node) == NT_NON_SWITCH_SECTION )))
					{
						for (j = node_ordinal + 1; j < node_count; j++)
						{
							if (gn_type(gr_node(route_index,j)) == NT_SWITCH_SECTION)
							{
								result = CI_TRUE;
								break;
							}
...
{
	int16_t approach_section = NO_INDEX, first_section = NO_INDEX, second_section = NO_INDEX,third_section = NO_INDEX;
			...
			switch (gr_state_machine(route_index))
			{
				...
				case RSM_TRAIN_IN_POSSIBLE:
					...
					if (IsFALSE(is_approach_cleared(route_index,CI_TRUE))
						&& (SCS_CLEARED == gn_section_state(first_section))
						&& (SCS_CLEARED != gn_section_state(second_section))
						&& (SCS_CLEARED != gn_section_state(third_section)))
					{
						/*车列异常驶入*/						
						if (RT_SHUNTING_ROUTE == gr_type(route_index))
						{
							if (NT_SWITCH_SECTION == gn_type(third_section))
							{
								sr_state_machine(route_index,RSM_TRAIN_IN_POSSIBLE);
							}
						}
