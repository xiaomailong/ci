...
int32_t CIInter_Init(void)
{
	...
	for (i = 0; i < MAX_SEMI_AUTO_BLOCK;i++)
	{
		...
		semi_auto_block_config[i].state = SAB_ERROR;
	...
	memset(train_loss_in_section,0xFF,sizeof(train_loss_in_section));
	...
	for (i = 0; i < MAX_SPECIAL_INPUT; i++)
	{
		...
		special_input_config[i].OutputNodeExist = CI_FALSE;
		...
		special_output_config[i].OutputNodeExist = CI_FALSE;
	}
	...
	{
		memset(CrossStation1SendToStart[i].LinkSignal,0x00,sizeof(CrossStation1SendToStart[i].LinkSignal));
		CrossStation1SendToStart[i].StartSignal = NO_INDEX;
		CrossStation1SendToStart[i].RouteState = RS_ERROR;
		memset(CrossStation2SendToStart[i].LinkSignal,0x00,sizeof(CrossStation2SendToStart[i].LinkSignal));
		CrossStation2SendToStart[i].StartSignal = NO_INDEX;
		CrossStation2SendToStart[i].RouteState = RS_ERROR;
		memset(CrossStation1RecviceFromEnd[i].LinkSignal,0x00,sizeof(CrossStation1RecviceFromEnd[i].LinkSignal));
		CrossStation1RecviceFromEnd[i].StartSignal = NO_INDEX;
		CrossStation1RecviceFromEnd[i].RouteState = RS_ERROR;
		memset(CrossStation2RecviceFromEnd[i].LinkSignal,0x00,sizeof(CrossStation2RecviceFromEnd[i].LinkSignal));
		CrossStation2RecviceFromEnd[i].StartSignal = NO_INDEX;
