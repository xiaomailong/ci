...
void search_route( void )
{
	...
	if (IsTRUE(can_build_route))
	{
		...
		if ((first_button >= 0) && (first_button <= TOTAL_NAMES))  
		{
			...
			{
				for(i = 0; (i < MAX_ILT_PER_ROUTES) && (ILT_routes[valid_ILT_index][i] != NO_INDEX); i++)
				{
					if (IsTRUE(is_button_locked(ILT[ILT_routes[valid_ILT_index][i]].start_button)))
					{
						CIHmi_SendNormalTips("按钮封锁：%s",gn_name(ILT[ILT_routes[valid_ILT_index][i]].start_button));
						result = CI_TRUE;
						break;
					}
					if (IsTRUE(is_button_locked(ILT[ILT_routes[valid_ILT_index][i]].end_button)))
					{
						CIHmi_SendNormalTips("按钮封锁：%s",gn_name(ILT[ILT_routes[valid_ILT_index][i]].end_button));
						result = CI_TRUE;
						break;
					}
				}
...
void set_route(int16_t ILT_routes[])
{
	...
	for (j = 0; j < route_count; j++)
	{
		...
		/*增加跨场作业标志*/
		if ((first_button != NO_INDEX) && (second_button != NO_INDEX) && (third_button == NO_INDEX) && (fourth_button == NO_INDEX) && (fifth_button != NO_INDEX))
		{
			if ((j == route_count - 1) && (routes[temp_index[j]].forward_route == NO_INDEX))
			{
				routes[temp_index[j]].other_flag = ROF_CROSS_STATION;
			}			
		}
