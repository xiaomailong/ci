...
void delete_route(route_t route_index)
{
	...
	if ( (route_index >= 0) && (routes[route_index].ILT_index != NO_INDEX))// && (gr_state(route_index) == RS_UNLOCKED))
	{
		...
		if (IsTRUE(belong_route) || (RS_UNLOCKED == gr_state(route_index)))
		{
...
uint16_t gn_data(node_t node_index)
{
	...
	else
	{
		...
		if (gn_type(node_index) == NT_SWITCH)
		{
			...
			{
				if(train_loss_in_section[i] == section)
				{
					lock_state = lock_state | 0x0040;					
					break;
				}
				if(i == MAX_TRAIN_LOSS - 1)
				{
					lock_state = lock_state & 0xFFBF;	
				}
			}
		}
		/*锁闭标志*/
		if ((gn_type(node_index) >= NT_SWITCH) && (gn_type(node_index) <= NT_SECTION))
		{
			...
			if(IsTRUE(is_section(node_index)))
			{
				for(i = 0; i < MAX_TRAIN_LOSS; i++)
				{
					if(train_loss_in_section[i] == node_index)
					{
						lock_state = lock_state | 0x0001;						
						break;
					}
...
{
	uint16_t temp1,temp2,section_state;	
			...
			if (node_index != NO_INDEX)
			{
				...
				if (IsTRUE(is_switch(node_index)))
				{
						...
						|| (temp1 == SWS_NORMAL) || (temp1 == SWS_REVERSE))
					{
						...
						{
							CIRemoteLog_Write("道岔%s#状态: OldValue:%d, NewValue:%d",gn_name(node_index),gn_switch_state(node_index),temp1);
						}
					...
					if (node_index2 != NO_INDEX)
					{
							...
							|| (temp1 == SWS_NORMAL) || (temp1 == SWS_REVERSE))
						{
							...
							{
								CIRemoteLog_Write("道岔%s#状态: OldValue:%d, NewValue:%d",gn_name(node_index2),gn_switch_state(node_index2),temp1);
							}
				...
				else if (IsTRUE(is_signal(node_index)))
				{
					...
					if (IsTRUE(is_shunting_signal(node_index)))
					{
							...
							|| (temp1 == SGS_B) || (temp1 == SGS_A))
						{
							...
							{
								CIRemoteLog_Write("信号%s状态: OldValue:%d, NewValue:%d",gn_name(node_index),gn_signal_state(node_index),temp1);
							}
						...
						if (node_index2 != NO_INDEX)
						{
								...
								|| (temp2 == SGS_B) || (temp2 == SGS_A))
							{
								...
								{
									CIRemoteLog_Write("信号%s状态: OldValue:%d, NewValue:%d",gn_name(node_index2),gn_signal_state(node_index2),temp2);
								}
				...
				else if (IsTRUE(is_section(node_index)))
				{
					...
					if ((temp1 == SCS_ERROR) || (temp1 == SCS_CLEARED) || (temp1 != SCS_CLEARED))
					{
						...
						{
							//CIRemoteLog_Write("%s状态:OldValue:%d,NewValue:%d",gn_name(node_index),gn_section_state(node_index),temp1);
						}
						...
						for (i = 0; i < MAX_SPECIAL_INPUT; i++)
						{
							...
							{
								if (IsTRUE(special_input_config[i].OutputNodeExist))
								{
									if (special_input_config[i].InputNodeIndex != NO_INDEX)
									{
											...
											&& (special_input_config[i].InputNodeState != 0))
										{
											...
											section_state = special_input_config[i].OutputNodeState;
										}
										else
										{
											//sn_state(node_index,SCS_OCCUPIED);
											section_state = SCS_OCCUPIED;
										}
									}
								}																
								break;
						...
						if (section_state == SCS_ERROR)
						{
							sn_state(node_index,temp1);
							if (temp1 != gn_section_state(node_index))
							{
								CIRemoteLog_Write("轨道%s状态: OldValue:%d, NewValue:%d",gn_name(node_index),gn_section_state(node_index),temp1);
							}
						}
						else
						{
							if ((temp1 == SCS_CLEARED) && (section_state == SCS_CLEARED))
							{
								sn_state(node_index,SCS_CLEARED);
								if (SCS_CLEARED != gn_section_state(node_index))
								{
									CIRemoteLog_Write("轨道%s状态: OldValue:%d, NewValue:%d",gn_name(node_index),gn_section_state(node_index),SCS_CLEARED);
								}
							}
							else
							{
								sn_state(node_index,SCS_OCCUPIED);
								if (SCS_OCCUPIED != gn_section_state(node_index))
								{
									CIRemoteLog_Write("轨道%s状态: OldValue:%d, NewValue:%d",gn_name(node_index),gn_section_state(node_index),SCS_OCCUPIED);
								}
							}
						}						
					}
					...
					if (node_index2 != NO_INDEX)
					{
						...
						if ((temp2 == SCS_ERROR) || (temp2 == SCS_CLEARED) || (temp2 != SCS_CLEARED))
						{
							...
							{
								//CIRemoteLog_Write("%s状态:OldValue:%d,NewValue:%d",gn_name(node_index2),gn_section_state(node_index2),temp2);
							}
							...
							if (section_state == SCS_ERROR)
							{
								...
								if (temp2 != gn_section_state(node_index2))
								{
									CIRemoteLog_Write("轨道%s状态: OldValue:%d, NewValue:%d",gn_name(node_index2),gn_section_state(node_index2),temp2);
								}
							}
							else
							{
								if ((temp2 == SCS_CLEARED) && (section_state == SCS_CLEARED))
								{
									sn_state(node_index2,SCS_CLEARED);
									if (SCS_CLEARED != gn_section_state(node_index2))
									{
										CIRemoteLog_Write("轨道%s状态: OldValue:%d, NewValue:%d",gn_name(node_index2),gn_section_state(node_index2),SCS_CLEARED);
									}
								}
								else
								{
									sn_state(node_index2,SCS_OCCUPIED);
									if (SCS_OCCUPIED != gn_section_state(node_index2))
									{
										CIRemoteLog_Write("轨道%s状态: OldValue:%d, NewValue:%d",gn_name(node_index2),gn_section_state(node_index2),SCS_OCCUPIED);
									}
								}
							}
						}
				...
				else if (gn_type(node_index) == NT_ZQD)
				{
					...
					if ((temp1 == SZQD_ERROR) || (temp1 == SZQD_HAS_SIGNAL) || (temp1 == SZQD_NO_SIGNAL))
					{
						...
						{
							CIRemoteLog_Write("总启动%s状态: OldValue:%d, NewValue:%d",gn_name(node_index),gn_state(node_index),temp1);
						}
				...
				else
				{
					...
					if ((temp1 == SIO_ERROR) || (temp1 == SIO_HAS_SIGNAL) || (temp1 == SIO_NO_SIGNAL))
					{
						if (temp1 != gn_state(node_index))
						{
							CIRemoteLog_Write("零散%s状态: OldValue:%d, NewValue:%d",gn_name(node_index),gn_state(node_index),temp1);
						}
						...
						{
							if ((special_input_config[i].InputNodeIndex != NO_INDEX)
								&& (special_input_config[i].InputNodeIndex == node_index))
							{
								if ((special_input_config[i].OutputNodeIndex != NO_INDEX)
									&& IsFALSE(special_input_config[i].OutputNodeExist))
								{
									if (special_input_config[i].OutputNodeState == SCS_CLEARED)
									{
										if (temp1 == special_input_config[i].InputNodeState )
										{
											sn_state(special_input_config[i].OutputNodeIndex,SCS_CLEARED);
										}
										else if (temp1 == SIO_ERROR)
										{
											sn_state(special_input_config[i].OutputNodeIndex,SCS_ERROR);
										}
										else
										{
											sn_state(special_input_config[i].OutputNodeIndex,SCS_OCCUPIED);
										}
									}
									else
									{
										sn_state(special_input_config[i].OutputNodeIndex,SIO_ERROR);
									}
								}																
								break;
							}
					...
					if (node_index2 != NO_INDEX)
					{
						...
						if ((temp2 == SIO_ERROR) || (temp2 == SIO_HAS_SIGNAL) || (temp2 == SIO_NO_SIGNAL))
						{
							...
							{
								CIRemoteLog_Write("零散%s状态: OldValue:%d, NewValue:%d",gn_name(node_index2),gn_state(node_index2),temp2);
							}
							...
							{
								if ((special_input_config[i].InputNodeIndex != NO_INDEX)
									&& (special_input_config[i].InputNodeIndex == node_index2))
								{
									if ((special_input_config[i].OutputNodeIndex != NO_INDEX)
										&& IsFALSE(special_input_config[i].OutputNodeExist))
									{
				...
				if (node_index2 != NO_INDEX)
				{
					...
					else
					{
						...
						if ((temp2 == SIO_ERROR) || (temp2 == SIO_HAS_SIGNAL) || (temp2 == SIO_NO_SIGNAL))
						{
							if (temp2 != gn_state(node_index2))
							{
								CIRemoteLog_Write("%s零散状态: OldValue:%d, NewValue:%d",gn_name(node_index2),gn_state(node_index2),temp2);
							}
							if ((temp2 == SIO_ERROR) && (gn_state(node_index2) != SIO_ERROR))
							{
								CIRemoteLog_Write("通信中断：%s",gn_name(node_index2));
		...
		for (i = 0; i < TOTAL_SIGNAL_NODE; i++)
		{
				...
				&& (((signal_nodes[i].input_address >> 2) & 0x3F) == EA))
			{
				...
				else
				{
					...
					/*特殊输入*/
					for (j = 0; j < MAX_SPECIAL_INPUT; j++)
					{
						if ((special_input_config[j].InputNodeIndex != NO_INDEX)
							&& (special_input_config[j].InputNodeIndex == node_index))
						{
							if ((special_input_config[j].OutputNodeIndex != NO_INDEX)
								&& IsFALSE(special_input_config[j].OutputNodeExist))
							{
								sn_state(special_input_config[j].OutputNodeIndex,SCS_ERROR);
							}																
							break;
						}
					}
...
void set_lock_button(int16_t button_index,CI_BOOL lock_flag)
{
	...
	if ((button_index >= TOTAL_SIGNAL_NODE) && (button_index < TOTAL_SIGNAL_NODE + TOTAL_BUTTONS))
	{
...
日期：     2015/09/14
****************************************************/
void set_train_loss_in_section(int16_t node_index,CI_BOOL lock_flag)
{
	int16_t i;
	...
	if ((node_index > 0) && (node_index < TOTAL_SIGNAL_NODE))
	{
		...
		if(IsTRUE(lock_flag))
		{
			for(i = 0; i < MAX_TRAIN_LOSS; i++)
			{
				if(train_loss_in_section[i] == NO_INDEX)
				{
					train_loss_in_section[i] = node_index;
					break;
				}
			}
		}
		...
		else
		{
			for(i = 0; i < MAX_TRAIN_LOSS; i++)
			{
				if((train_loss_in_section[i] != NO_INDEX) && (train_loss_in_section[i] == node_index))
				{
					train_loss_in_section[i] = NO_INDEX;
					break;
				}
			}
		}
...
日期：     2015/09/14
****************************************************/
CI_BOOL is_train_loss_in_section(int16_t node_index)
{
	CI_BOOL result = CI_FALSE;
	int16_t i;
