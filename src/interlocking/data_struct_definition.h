...
typedef struct /*特殊输入相关数据*/
{
	...
	CI_BOOL OutputNodeExist; /*本站设备存在*/
}special_input_t;
...
typedef struct /*跨场作业终端-->始端相关数据*/
{
	char_t LinkSignal[TEST_NAME_LENGTH]; /*对方站关联的信号机*/
	node_t StartSignal; /*本站关联的信号机*/
	EN_route_state RouteState; /*进路状态*/
}cross_station_end_start_t;

typedef struct /*跨场作业虚拟进路相关数据*/
{
	node_t StartSignal; /*本站关联的信号机*/
	node_t RouteSignal; /*虚拟进路始端信号机*/
	node_t RouteSection; /*虚拟进路轨道区段*/
	CI_BOOL IsExist;	/*是否存在虚拟进路*/
}cross_station_route_t;
