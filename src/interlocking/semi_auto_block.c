...
/****************************************************
函数名：   sab_start_recv_positive_signal
功能描述： 开始接收正信息
返回值：   CI_BOOL
参数：     int16_t index
作者：	   hejh
日期：     2015/08/19
****************************************************/
CI_BOOL sab_start_recv_positive_signal(int16_t index);
...
日期  ：   2015/08/19
****************************************************/
CI_BOOL sab_end_recv_positive_signal(int16_t index);

/****************************************************
函数名：   sab_start_recv_negative_signal
功能描述： 开始接收负信号
返回值：   CI_BOOL
参数：     int16_t index
作者：	   hejh
日期：     2015/08/19
****************************************************/
CI_BOOL sab_start_recv_negative_signal(int16_t index);

/****************************************************
函数名:    sab_end_recv_negative_signal
功能描述:  负信号接收完毕
返回值:    
参数:      int16_t index
作者  :    hejh
日期  ：   2015/08/19
****************************************************/
CI_BOOL sab_end_recv_negative_signal(int16_t index);
		...
		if(semi_auto_block_config[i].cancel_button == button_index)
		{
			...
			if (gsab_recieve_departure_state(i)== SAS_DEPARTURE )
			{
				if ((sab_state == SAB_REQUEST_BLOCK) 
					...
					|| (sab_state == SAB_START_RECIEVED_AUTO_REPLY) 
					|| (sab_state == SAB_RECIEVED_AUTO_REPLY) 
					|| (sab_state == SAB_FIRST_SECTION_CLEAR1)
					|| (sab_state == SAB_RECIEVED_AGREEMENT)
					|| (sab_state == SAB_FIRST_SECTION_CLEAR2))
				{
			...
			if (gsab_recieve_departure_state(i)== SAS_RECIEVING)
			{
				...
				if ((sab_state == SAB_START_RECIEVED_DEPARTED) || (sab_state == SAB_RECIEVED_DEPARTED))
				{
					CIHmi_SendNormalTips("对方站列车已出发：%s",gn_name(semi_auto_block_config[i].mem_node));
				}
				if ((sab_state == SAB_TRAIN_APPROACH) || (sab_state == SAB_TRAIN_IN_ENTRY_SIGNAL))
				{
...
void semi_auto_block_process(void)
{
	...
	{
		/*半自动未启用时接收到信息*/
		if(gsab_state(i) == SAB_ERROR)
		{
			if(IsTRUE(sab_start_recv_positive_signal(i))) 
			{
				CIHmi_SendDebugTips("【%s】半自动未启用时开始接收(+)信息",gn_name(semi_auto_block_config[i].mem_node));
				ssab_state(i,SAB_START_RECIEVED_POSITIVE_MESSAGE);
			}
			if (IsTRUE(sab_start_recv_negative_signal(i)))
			{
				CIHmi_SendDebugTips("【%s】半自动未启用时开始接收(-)信息",gn_name(semi_auto_block_config[i].mem_node));
				ssab_state(i,SAB_START_RECIEVED_NEGATIVE_MESSAGE);
			}
		}
		if(gsab_state(i) == SAB_START_RECIEVED_POSITIVE_MESSAGE)
		{
			if(IsTRUE(sab_end_recv_positive_signal(i))) 
			{
				CIHmi_SendDebugTips("【%s】半自动未启用时(+)信息接收完毕",gn_name(semi_auto_block_config[i].mem_node));
				ssab_state(i,SAB_ERROR);
			}
		}
		if(gsab_state(i) == SAB_START_RECIEVED_NEGATIVE_MESSAGE)
		{
			if (IsTRUE(sab_end_recv_negative_signal(i)))
			{
				CIHmi_SendDebugTips("【%s】半自动未启用时(-)信息接收完毕",gn_name(semi_auto_block_config[i].mem_node));
				ssab_state(i,SAB_ERROR);
			}
		...
		if ((gsab_recieve_departure_state(i) == SAS_NORMAL) 
			&& (gsab_state(i) == SAB_NORMAL) 
			&& IsTRUE(sab_start_recv_positive_signal(i)))
		{
			CIHmi_SendDebugTips("【%s】本站开始接收闭塞请求",gn_name(semi_auto_block_config[i].mem_node));
			ssab_state(i,SAB_START_RECIEVED_REQUEST);
...
void recieving_semi_auto(int16_t i)
{
	switch(gsab_state(i))
	{
		case SAB_START_RECIEVED_REQUEST:     /*对方站请求闭塞接收完毕*/
			if (IsTRUE(sab_end_recv_positive_signal(i)))
			{
				CIHmi_SendDebugTips("【%s】对方站请求闭塞接收完毕",gn_name(semi_auto_block_config[i].mem_node));
		...
		case SAB_RECIEVED_REQUEST:     /*收到(+)/对方站请求闭塞*/
			...
			break;
		case SAB_AGREED_RECIEVE_TRAIN: /*开始接收复原信息*/
			if(IsTRUE(sab_start_recv_negative_signal(i)))
			{
				CIHmi_SendDebugTips("【%s】开始接收对方站复原信息",gn_name(semi_auto_block_config[i].mem_node));
				ssab_state(i,SAB_RECV_START_RECIEVED_CANCEL);
			}
			break;
		case SAB_RECV_START_RECIEVED_CANCEL: /*复原信息接收完毕*/
			if(IsTRUE(sab_end_recv_negative_signal(i)))
			{
				CIHmi_SendDebugTips("【%s】对方站复原信息接收完毕",gn_name(semi_auto_block_config[i].mem_node));
				ssab_state(i,SAB_RECV_RECIEVED_CANCEL);
			}
			break;
		case SAB_RECV_RECIEVED_CANCEL: /*解除闭塞*/
		...
		case SAB_SENDING_AGREEMENT:    
			...
			break;
		case SAB_SENT_AGREEMENT:       /*对方站通知出发*/ 
			if(IsTRUE(sab_start_recv_positive_signal(i))) 
			{
				CIHmi_SendDebugTips("【%s】开始接收对方站通知出发信息",gn_name(semi_auto_block_config[i].mem_node));
				ssab_state(i,SAB_START_RECIEVED_DEPARTED);
			}
			if(IsTRUE(sab_start_recv_negative_signal(i)))/*接收到复原信息*/
			{
				CIHmi_SendDebugTips("【%s】开始接收对方站复原信息",gn_name(semi_auto_block_config[i].mem_node));
				ssab_state(i,SAB_RECV_START_RECIEVED_CANCEL);
			}
			break;
		case SAB_START_RECIEVED_DEPARTED:       /*通知出发信息接收完毕*/ 
			if(IsTRUE(sab_end_recv_positive_signal(i))) 
			{
				CIHmi_SendDebugTips("【%s】对方站通知出发信息接收完毕",gn_name(semi_auto_block_config[i].mem_node));
				ssab_state(i,SAB_RECIEVED_DEPARTED);
			}
			break;
		case SAB_RECIEVED_DEPARTED:       /*列车接近*/ 
			if(IsTRUE(sab_train_arrive_approach(i)))// && IsTRUE(sab_train_signal_opened(i))) 
			{
				CIHmi_SendDebugTips("【%s】列车接近",gn_name(semi_auto_block_config[i].mem_node));
				ssab_state(i,SAB_TRAIN_APPROACH);
			}
			break;
		case SAB_TRAIN_APPROACH:       /*列车进入进站信号机内方/列车已到达*/
...
void departure_semi_auto(int16_t i)
{
	switch(gsab_state(i))
	{
		...
		case SAB_REQUEST_BLOCK:        /*按压闭塞按钮/本站请求闭塞*/      
			...
			break;
		case SAB_SENT_REQUEST: /*开始接收自动回执信息	*/        
			if (IsTRUE(sab_start_recv_negative_signal(i))) /*收到（—）/对方站已收到请求信息*/
			{
				CIHmi_SendDebugTips("【%s】本站开始接收自动回执信息",gn_name(semi_auto_block_config[i].mem_node));
				ssab_state(i,SAB_START_RECIEVED_AUTO_REPLY);
			} 
			break;
		case SAB_START_RECIEVED_AUTO_REPLY: /*接收自动回执信息*/        
			if (IsTRUE(sab_end_recv_negative_signal(i))) /*收到（—）/对方站已收到请求信息*/
			{
				CIHmi_SendDebugTips("【%s】自动回执信息接收完毕",gn_name(semi_auto_block_config[i].mem_node));
				ssab_state(i,SAB_RECIEVED_AUTO_REPLY);
			} 
			break;
		case SAB_RECIEVED_AUTO_REPLY: /*检查进站信号机内方第一区段*/        
			if (IsTRUE(sab_first_section_clear(i))) 
			{
				CIHmi_SendDebugTips("【%s】进站信号机内方第一区段空闲1",gn_name(semi_auto_block_config[i].mem_node));
				ssab_state(i,SAB_FIRST_SECTION_CLEAR1);
			} 
			break;
		case SAB_FIRST_SECTION_CLEAR1:  /*开始接收正信号*/
			if (IsTRUE(sab_start_recv_positive_signal(i)))
			{
				CIHmi_SendDebugTips("【%s】本站开始接收同意接车信息",gn_name(semi_auto_block_config[i].mem_node));
				ssab_state(i,SAB_START_RECIEVED_AGREEMENT);
			} 
			break;
		case SAB_START_RECIEVED_AGREEMENT:  /*接收正信号*/
			if (IsTRUE(sab_end_recv_positive_signal(i)))
			{
				CIHmi_SendDebugTips("【%s】同意接车信息接收完毕",gn_name(semi_auto_block_config[i].mem_node));
				ssab_state(i,SAB_RECIEVED_AGREEMENT);
			} 
			break;
		case SAB_RECIEVED_AGREEMENT:   	/*检查进站信号机内方第一区段*/
			if (IsTRUE(sab_first_section_clear(i)))
			{
				CIHmi_SendDebugTips("【%s】进站信号机内方第一区段空闲2",gn_name(semi_auto_block_config[i].mem_node));
				ssab_state(i,SAB_FIRST_SECTION_CLEAR2);
			} 
			else
			{
				CIHmi_SendDebugTips("【%s】进站信号机内方第一区段占用2",gn_name(semi_auto_block_config[i].mem_node));
				ssab_state(i,SAB_RECIEVED_AUTO_REPLY);
			}
			break;
		case SAB_FIRST_SECTION_CLEAR2:   	/*发车信号机已开放*/
			...
			break;
		case SAB_OUT_SIGNAL_OPENED:    /*列车压入进站信号机内方第一区段*/
			if (IsTRUE(sab_out_signal_has_closed(i)))
			{
				CIHmi_SendDebugTips("【%s】列车压入进站信号机内方第一区段",gn_name(semi_auto_block_config[i].mem_node));
				ssab_state(i,SAB_DEPARTED);
			}
			if (IsTRUE(sab_start_recv_negative_signal(i)))
			{
				CIHmi_SendDebugTips("【%s】本站开始接收复原信息",gn_name(semi_auto_block_config[i].mem_node));
				ssab_state(i,SAB_DEPA_START_RECIEVED_CANCEL);
			} 
			break;
		case SAB_DEPARTED:             /*发送通知出发信息（+）*/   	
			if (IsTRUE(sab_send_positive_signal(i)))
			{
				CIHmi_SendDebugTips("【%s】本站已发送通知出发信息",gn_name(semi_auto_block_config[i].mem_node));
				ssab_state(i,SAB_SENT_DEPARTED);
			}
			break;
		case SAB_SENT_DEPARTED:        /*开始接收复原信息（-）*/
			if (IsTRUE(sab_start_recv_negative_signal(i)))
			{
				CIHmi_SendDebugTips("【%s】本站开始接收复原信息",gn_name(semi_auto_block_config[i].mem_node));
				ssab_state(i,SAB_DEPA_START_RECIEVED_CANCEL);
			...
			break;
		case SAB_DEPA_START_RECIEVED_CANCEL:        /*复原信息接收完毕*/
			if (IsTRUE(sab_end_recv_negative_signal(i)))
			{
				CIHmi_SendDebugTips("【%s】复原信息接收完毕",gn_name(semi_auto_block_config[i].mem_node));
				ssab_state(i,SAB_DEPA_RECIEVED_CANCEL);
			} 
			break;
		case SAB_DEPA_RECIEVED_CANCEL:      /*接收到解除闭塞信息/已收到解除闭塞信息*/
			ssab_state(i,SAB_NORMAL);
			CIHmi_SendDebugTips("【%s】对方站解除闭塞",gn_name(semi_auto_block_config[i].mem_node));
			break;
		default:
...
CI_BOOL sab_out_signal_has_closed(int16_t index)
{
	...
	if (semi_auto_block_config[index].belong_route != NO_INDEX )
	{
		...
		//CIHmi_SendDebugTips("半自动进路状态0：%d",gr_state(semi_auto_block_config[index].belong_route));
		/*压入发车进路的最后一个区段*/
		if ((gr_state(semi_auto_block_config[index].belong_route) == RS_SK_N_SIGNAL_CLOSED)
		...
		{			
			...
			{
				//CIHmi_SendDebugTips("半自动进路状态1：%d",gr_state(semi_auto_block_config[index].belong_route));
				result = CI_TRUE;
			}
		}
		if ((gr_state(semi_auto_block_config[index].belong_route) != RS_A_SIGNAL_CLOSED)
			&& (gr_state(semi_auto_block_config[index].belong_route) != RS_FAILURE))
		{			
			if (gn_section_state(gr_last_section(semi_auto_block_config[index].belong_route)) != SCS_CLEARED)
			{
				//CIHmi_SendDebugTips("半自动进路状态2：%d",gr_state(semi_auto_block_config[index].belong_route));
				//result = CI_TRUE;
			}
		}
	}
